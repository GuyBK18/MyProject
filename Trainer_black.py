...
def main ():
    player1 = DQN_Agent(player=-1, env=environment,parametes_path=None)
    player_hat = DQN_Agent(player=-1, env=environment, train=False)
    Q = player1.DQN
    Q_hat = Q.copy()
    Q_hat.train = False
    player_hat.DQN = Q_hat
    player2 = Random_Agent(env=environment, player=1)   
    ...
    best_res = -200
    loss_count = 0
    tester = Tester(player1=player1, player2=Random_Agent(env=environment, player=1), env=environment)
